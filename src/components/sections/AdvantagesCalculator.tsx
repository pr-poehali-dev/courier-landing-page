...
import { ADVANTAGES, REGIONS, PLATFORMS, type PlatformId } from "./data";
...
export function CalculatorSection() {
  ...
  const earned = Math.round(region.base * platform.mult * (userHours / maxHours));
  const earnedFormatted = earned.toLocaleString("ru-RU");
  const perDay = Math.round(earned / days).toLocaleString("ru-RU");
  const maxEarned = Math.round(region.base * platform.mult);
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-5 gap-8 ${inView ? "animate-fade-up delay-200" : "opacity-0"}`}>
          ...
          <div className="lg:col-span-3 bg-white/5 backdrop-blur-sm border border-white/10 rounded-3xl p-8 space-y-8">
            ...
            {/* Platform switcher */}
            <div>
              <label className="font-semibold text-white text-sm mb-3 block">Платформа</label>
              <div className="flex gap-3">
                {PLATFORMS.map((p) => (
                  <button
                    key={p.id}
                    onClick={() => setPlatformId(p.id)}
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-2xl font-semibold text-sm transition-all duration-200"
                    style={
                      platformId === p.id
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <span className="font-display text-xl font-bold" style={{ color: platform.color }}>{hours} ч</span>
              </div>
              <input
                type="range" min={2} max={12} step={1} value={hours}
                onChange={(e) => setHours(Number(e.target.value))}
                className="w-full"
                style={{ background: `linear-gradient(to right, ${platform.color} ${((hours - 2) / 10) * 100}%, #374151 ${((hours - 2) / 10) * 100}%)` }}
              />
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <span className="font-display text-xl font-bold" style={{ color: platform.color }}>{days} дн</span>
              </div>
              <input
                type="range" min={5} max={28} step={1} value={days}
                onChange={(e) => setDays(Number(e.target.value))}
                className="w-full"
                style={{ background: `linear-gradient(to right, ${platform.color} ${((days - 5) / 23) * 100}%, #374151 ${((days - 5) / 23) * 100}%)` }}
              />
              <div className="flex justify-between text-xs text-gray-500 mt-1">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="text-xs text-white/70 mt-0.5">₽ в день</div>
                </div>
                <div>
                  <div className="font-display text-2xl font-bold text-white">{hours * days}</div>
                  <div className="text-xs text-white/70 mt-0.5">часов итого</div>
                </div>
              </div>
            </div>

            <div className="bg-white/5 border border-white/10 rounded-2xl p-5 text-center">
              <div className="text-gray-400 text-xs mb-1">Максимум в {region.name} · {platform.label}</div>
              <div className="font-display text-2xl font-bold text-white">{maxEarned.toLocaleString("ru-RU")} ₽</div>
